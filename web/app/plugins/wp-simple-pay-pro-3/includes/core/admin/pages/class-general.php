				...
				if ( 'general' == $section ) {
					...
					$fields[ $section ] = array(
						...
						'cancelled_page' => array(
							'title'       => esc_html__( 'Payment Cancelled Page', 'simple-pay' ),
							'type'        => 'select',
							'name'        => 'simpay_' . $this->option_group . '_' . $this->id . '[' . $section . '][cancelled_page]',
							'id'          => 'simpay-' . $this->option_group . '-' . $this->id . '-' . $section . '-cancelled-page',
							'value'       => '' === $this->get_option_value( $section, 'cancelled_page' )
								? $this->get_option_value( $section, 'failure_page' )
								: $this->get_option_value( $section, 'cancelled_page' ),
							'page_select' => 'page_select',
							'class'       => array( 'simpay-chosen-search' ),
							'default'     => $failure_default,
							'description' => esc_html__( 'The page customers are sent to after a cancelling a Stripe.com Checkout Session.', 'simple-pay' ),
						),
					);
					...
					$fields[ $section ] = array(
						'default_plugin_styles' => array(
							'title'       => esc_html__( 'Opinionated Styles', 'simple-pay' ),
							'type'        => 'radio',
							...
							'description' => sprintf( esc_html__( 'Automatically apply %1$s styles to payment form fields and buttons.', 'simple-pay' ), SIMPLE_PAY_PLUGIN_NAME ) . '<br />' . esc_html__( 'Styles on the Stripe.com Checkout page cannot be changed.', 'simple-pay' ),
						),
					);

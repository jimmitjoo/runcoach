...
class Menus {
	...
	public function __construct() {
		...
		add_action( 'admin_menu', array( $this, 'add_external_upgrade_url_to_menu_item' ) );
	...
	/**
	 * This function changes the menu highlight for our Add New submenu item.
	 * We check for the page query arg and make sure it's ours before modifying it.
	 *
	 * @since 3.0.0
	 *
	...
	public static function add_menu_items() {
		...
		add_submenu_page(
			'simpay',
			'Simple Pay',
			__( 'Add New', 'simple-pay' ),
			'manage_options',
			'simpay&action=create',
			function () {
			}
		);
	...
	/**
	 * Updates the "Upgrade to Pro" menu item's URL to an external link.
	 *
	 * @since 3.7.0
	 */
	public function add_external_upgrade_url_to_menu_item() {
		global $submenu;

		$submenu['simpay'][4] = array(
			__( 'Upgrade to Pro', 'simple-pay' ),
			'manage_options',
